...
try:
    for i in range(1, 152): # como eu deixo isso dinamico? caso mude a quantidade de um dia pro outro
        ...
        evolution = get_json(url_evolution)
        ...
        encounters = get_json(url_encounters)
        create_file(file_name, encounters_dir, encounters)

        logging.info(f'File {file_name} exported to ENCOUNTERS folder.')

        for i in pokemon["types"]:
            type_id = i["type"]["url"].split("/")[-2]
            url_type = f"https://pokeapi.co/api/v2/type/{type_id}"
            type = get_json(url_type)
            create_file(type_id, type_dir, type)

        logging.info(f'File {file_name} exported to TYPE folder.')
